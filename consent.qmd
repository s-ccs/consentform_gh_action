...
```{python,setup}
import tomli
with open("_pyproject.toml", "rb") as f:
    toml = tomli.load(f)
...
`{python} toml['Consentform']['study_purpose_de']`
:::
::: {.content-visible when-profile="en"}
## 1) **Purpose of the study**

`{python} toml['Consentform']['study_purpose_en']`
:::
...
`{python} ifno(toml['Consentform']['use_eeg'], ":::: {.content-hidden}")`
::: {.content-visible when-profile="de"}
...
:::
`{python} ifno(toml['Consentform']['use_eeg'], "::::")`



`{python} ifno(toml['Consentform']['use_eyetracking'], ":::: {.content-hidden}")`
::: {.content-visible when-profile="de"}
...
:::
`{python} ifno(toml['Consentform']['use_eyetracking'],"::::")`



::: {.content-visible when-profile="de"}
1. `{python} toml['Consentform']['study_task_de']`
:::
::: {.content-visible when-profile="en"}
1. `{python} toml['Consentform']['study_task_en']`
:::
...
::: {.content-visible when-profile="de"}
Inklusive der Fragebögen und Vorbereitungen, dauert der Versuch ca. `{python} toml['Consentform']['study_duration_hrs']` Stunden.
:::
::: {.content-visible when-profile="en"}
Including the questionnaires and preparations, the experiment takes about `{python} toml['Consentform']['study_duration_hrs']` hours.
:::
...
According to current knowledge, this study is harmless and painless for the participants. By participating in this study, you are not exposing yourself to any particular risks and there are no known side effects. `{python} ifyes(toml['Consentform']['use_vr'], "In the case of virtual reality experiments, exposure may cause motion sickness or eye strain.")` However, because this study is new in its entirety, the occurrence of as-yet-unknown side effects cannot be completely ruled out. 
...
Die Studie wird mit `{python} toml['Consentform']['pay_per_hour']` € pro Stunde vergütet, abgerechnet nach angefangenen halben Stunden
:::
::: {.content-visible when-profile="en"}
## 6) **Remuneration**

The study will be remunerated with `{python} toml['Consentform']['pay_per_hour'] € per hour, with payment rounded up to the nearest half hour. 
:::
